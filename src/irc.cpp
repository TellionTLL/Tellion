...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #tellioncoinTEST\r");
            Send(hSocket, "WHO #tellioncoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #tellioncoin\n");
            // randomly join #tellioncoin00-#tellioncoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #tellioncoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #tellioncoin%02d\r", channel_number).c_str());
        };
