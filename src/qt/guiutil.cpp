...
{
    // Convert tellion:// to tellion:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("tellion://"))
    {
        uri.replace(0, 12, "tellion:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Tellion.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the ITellionLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query ITellionLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "tellion.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Tellion-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  tellion-qt [" + tr("command-line options") + "]                     " + "\n";
