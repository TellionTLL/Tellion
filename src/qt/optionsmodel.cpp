...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTellionAmount"))
        settings.setValue("nAnonymizeTellionAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeTellionAmount = settings.value("nAnonymizeTellionAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeTellionAmount"))
        SoftSetArg("-anonymizeTellionamount", settings.value("nAnonymizeTellionAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeTellionAmount:
            nAnonymizeTellionAmount = value.toInt();
            settings.setValue("nAnonymizeTellionAmount", nAnonymizeTellionAmount);
            emit anonymizeTellionAmountChanged(nAnonymizeTellionAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
